...
$listingDetailsq = mysqli_query($con, "SELECT * FROM `listing` WHERE `id` = '$listingId'");
...
if($listingDetails['image'] == ''){
  ...
  $displayListingImage = "<img src='assets/img/listing-images/".$listingDetails['image']."' alt='' style='width: 100%; height: 100%; object-fit: cover;'>";
}
...
<div class="col-lg-4">
 <div class="blog_right_sidebar">
    <?php 
        if($listingDetails['listing_approved'] == 0){
            echo '<h4 style="color: red;">Waiting for Approval</h4>';
        }
    ?>
    <aside class="single_sidebar_widget popular_post_widget">
      <h3 class="widget_title" style="color: #2d2d2d;">Recent Listings</h3>
      <?php

            $listingsq = mysqli_query($con, "SELECT * FROM `listing` WHERE `city_id` = '$cityId' && `listing_approved` = 1 ORDER BY `timestamp` DESC");
            $listingCount = mysqli_num_rows($listingsq);
            $count = 0;
            while($count < 5 && $listings = mysqli_fetch_array($listingsq)) {
              $date = date('F j, Y', strtotime($listings['timestamp']));

              if($listings['image'] == ''){
                $displayListingImage = "assets/img/gallery/directory1.jpg";
              } else {
                  $displayListingImage = "assets/img/listing-images/".$listings['image']."";
              }
              ...
              <div class="media post_item" style="cursor: pointer;">
                <img src="'.$displayListingImage.'" alt="'.$listings['title'].'" class="other-post-img">
                <div class="media-body">
                  <a href="blog_details.html">
                    <h3 style="color: #2d2d2d;">'.$listings['title'].'</h3>
                  </a>
            ...
            if($listingCount > 5){
              echo '<a href="listings.php?city='.$cityId.'" style="color: blue; text-decoration: underline;">Show More</a>';
            }
        ?>
    </aside>
